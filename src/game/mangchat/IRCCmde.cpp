            ...
            if(!AcctIsLoggedIn(_PARAMS[0].c_str()))
            {
                ...
                if (result)
                {
                    ...
                    if(GMLevel >= 0)
                    {
                        ...
                        Send_IRCA(CD->USER, MakeMsg("You Are Now Logged In As %s, Welcome To WoWPwned Chat Admin Mode.", _PARAMS[0].c_str()), true, CD->TYPE);
    ...
    if(result)
    {
        if(IsLoggedIn(CD->USER))
        {
            if(_PARAMS[0] == "")
            {
                ...
                if(result)
                {
                    std::string output = "\002WoWPwned IRC Commands:\017 ";
...
void IRCCmd::Sysmsg_Server(_CDATA *CD)
{
	...
	if(_PARAMS[0] == "a")
    {
        ...
        std::string ancmsg = MakeMsg("\00304,08\037/!\\\037\017\00304 WoW Pwned Alert \00304,08\037/!\\\037\017 %s",_PARAMS[1].c_str());
    ...
    else if (_PARAMS[0] == "e")
    {
        ...
        std::string notmsg = MakeMsg("\00304,08\037/!\\\037\017\00304 WoW Pwned Event \00304,08\037/!\\\037\017 %s",_PARAMS[1].c_str());
    ...
    else if (_PARAMS[0] == "n")
    {
        ...
        std::string notmsg = MakeMsg("\00304,08\037/!\\\037\017\00304 WoW Pwned Notification \00304,08\037/!\\\037\017 %s",_PARAMS[1].c_str());
